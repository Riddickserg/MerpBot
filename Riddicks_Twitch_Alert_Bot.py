                ...
                for streamer in data["streamers"]:
                    ...
                    async with session.get(url, headers=headers) as resp:
                        ...
                        if stream_live and not streamer.get("is_live"):
                            try:
                                embed_color = int(streamer.get("color", "#FF0000").lstrip("#"), 16)
                            except ValueError:
                                embed_color = 0xFF0000
                            
                            embed = discord.Embed(
                                title=f"{user} is LIVE om Twitch!",
                                description=streamer["message"],
                                color=embed_color,
                                url=streamer["link"]
                            )
                            embed.set_thumbnail(url="https://static-cdn.jtvnw.net/emoticons/v2/300141381/static/light/3.0")
                            embed.set_image(url=f"https://static-cdn.jtvnw.net/previews-ttv/live_user_{user.lower()}-640x360.jpg")
                            embed.set_footer(text="Click the title to watch!", icon_url="https://static.twitchcdn.net/assets/favicon-32-e29e246c157142c94346.png")
                            ...
                            try:
                                ...
                                if msg_id:
                                    ...
                                    offline_embed = discord.Embed(
                                        title=f"{user} has gone offline.",
                                        colour=discord.Color.dark_grey()
                                    )
                                    await old_msg.edit(embe=offline_embed)
